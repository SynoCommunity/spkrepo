...
def upgrade():
    ...
    architecture_table = op.create_table('architecture',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('code', sa.Unicode(length=20), nullable=False),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('code')
    )
    op.bulk_insert(architecture_table, [
        {'code': 'noarch'},
        {'code': 'ppc824x'},
        {'code': 'ppc854x'},
        {'code': 'ppc853x'},
    ...
    firmware_table = op.create_table('firmware',
    sa.Column('id', sa.Integer(), nullable=False),
    ...
    with op.batch_alter_table('version', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_version_version'), ['version'], unique=False)

    op.create_table('package_user_maintainer',
    sa.Column('package_id', sa.Integer(), nullable=True),
    sa.Column('user_id', sa.Integer(), nullable=True),
    sa.ForeignKeyConstraint(['package_id'], ['package.id'], ),
    sa.ForeignKeyConstraint(['user_id'], ['user.id'], )
    )
    op.create_table('version_service_dependency',
    ...
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('version_id', sa.Integer(), nullable=False),
    sa.Column('size', sa.Enum('72', '120', '256', name='icon_size'), nullable=False),
    sa.Column('path', sa.Unicode(length=100), nullable=False),
    sa.ForeignKeyConstraint(['version_id'], ['version.id'], ),
    sa.PrimaryKeyConstraint('id'),
    ...
    op.create_table('build_architecture',
